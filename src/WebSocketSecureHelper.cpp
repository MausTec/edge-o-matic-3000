...
namespace WebSocketSecureHelper {
  ...
  void registerRemote(esp_websocket_client_handle_t client) {
    // RemoteHandler *bridge = BridgeHandler::create(client);
    // ESP_LOGE(TAG, "Registered Bridge.");
  }

  void send(int num, std::string payload) {
    if (num >= 0) {
      RemoteHandler *c = (RemoteHandler*) activeClients[num];
      if (c != nullptr)
        c->sendText(payload.c_str());
    } else {
      for (int i = 0; i < MAX_CLIENTS; i++) {
        send(i, payload.c_str());
      }
    }
  }
  ...
  //   res->setHeader("Location", (std::to_string("//nogasm-ui.maustec.io/#") + WiFi.localIP().std::tostring()).c_str());
  ...
  RemoteHandler *RemoteHandler::create() {
  ...
  BridgeHandler *BridgeHandler::create(esp_websocket_client_handle_t client) {
    BridgeHandler *handler = new BridgeHandler(client);
    for (int i = 0; i < MAX_CLIENTS; i++) {
      if (activeClients[i] == nullptr) {
        activeClients[i] = handler;
        break;
  ...
  // other clients
  void RemoteHandler::onMessage(void *inbuf) {
  ...
  void RemoteHandler::sendText(const char* payload) {
    // if (this->closed()) {
    //   this->onClose();
    //   return;
    // }
  ...
  void BridgeHandler::sendText(const char* payload) {
