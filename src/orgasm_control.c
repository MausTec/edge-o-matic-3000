...
void orgasm_control_init(void) {
    ...
    config_state.motor_max_speed = 0;
    config_state.motor_start_speed = 0;
    config_state.motor_ramp_time_s = 0;
...
static void orgasm_control_updateEdgingTime() { // Edging+Orgasm timer
    ...
    if (orgasm_control_is_permit_orgasm_reached() && !orgasm_control_is_post_orgasm_reached()) {
        ...
        // gradualy raise motor speed to max speed. protect not to go higher than max
        increment_motor();
    }
...
void orgasm_control_tick() {
    ...
    if (millis - arousal_state.last_update_ms > update_frequency_ms) {
        ...
        snprintf(
            ...
            post_orgasm_state.clench_pressure_threshold,
            post_orgasm_state.clench_duration_millis,
            denial_state.denial_count,
            post_orgasm_state.edge_count_to_orgasm
        );
...
//        if (state.denial_count >= post_orgasm_state.edge_count_to_orgasm) {
//            return ocTRUE;
//        } else {
//            return ocFALSE;
//        }
