...
namespace UpdateHelper {
  ...
  size_t waitUntilAvailable(FILE *stream, long timeout_ms = 10000) {
    // long start_ms = esp_timer_get_time() / 1000UL;
    size_t available = 0;
    // while(available == 0 && (esp_timer_get_time() / 1000UL) - start_ms < timeout_ms) {
