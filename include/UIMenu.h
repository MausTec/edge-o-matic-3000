 ...
 * bounce that sometimes happens when you're in-between detents on the scroll wheel. Is this an
 * issue with the PEL12T? The no-name (also a PEL12 variant) SparkFun encoder does this too. I think
 * every encoder in the universe does this (see CR-10).
 *
 * Maybe I just need to learn patience when scrolling.
 * That would explain why the scroll wheel on my mouse is wearing out.
 *
 * But look, that scroll wheel has seen the duality of heal staff and murder gun when I mained
 * Mercy, and that is a pretty tense environment when I'm furiously scrolling to grab my pistol and
 * shoot a Junkrat for--     what was I on about?
 *
...
class UIMenu {
  public:
    ...
    void addItem(std::string text, ParameterizedMenuCallback pcb = nullptr, void* arg = nullptr) {
    ...
    // Selection Handling
    virtual void selectNext(int steps);
    virtual void selectPrev(int steps);
